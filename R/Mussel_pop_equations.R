...
Mussel_pop_equations <- function(Param, N, Tint, Phyint, DTint, POCint, Ccont, Ncont, Pcont, POMint, TSSint, Wb,R,t,trip){
  ...
  Wd=Wb+R
    ...
    I_P=(CRmax*fa*Wb^q)*(DTint*Pcont+Phyint*Pcont)*N/1e6     # Daily P ingestion P [kg/d]
    I_N=(CRmax*fa*Wb^q)*(DTint*Ncont+Phyint*Ncont)*N/1e6     # Daily N ingestion [kg/d]
    I_C=(CRmax*fa*Wb^q)*(DTint*Ccont+Phyint*Ccont)*N/1e6     # Daily C ingestion [kg/d]
    ...
    if (Aing<(Amax*fa*Wd^gamma))  {
      ...
      pseudof = 0.0                                  # Pseudofecies production [J/d]
      psC = 0.0                                      # pseudofecies C content [kg/d]
      psN = 0.0                                      # pseudofecies N content [kg/d]
      psP = 0.0                                      # pseudofecies P content [kg/d]

    } else  {

      A=Amax*fa*Wd^gamma
      Imass=(CRmax*fa*Wb^q)*(POCint)       # Daily POC ingestion [mg/d]
      ratio=NH4/(Imass*AE*Ncont/10^6)      # Excreted/ingested N [-]
      pseudof = (Aing-A)/Epspsf            # Pseudofecies production [g/d]
      psC = pseudof*Ccont*N/1e3              # pseudofecies C content [kg/d]
      psN = pseudof*Ncont*N/1e3              # pseudofecies N content [kg/d]
      psP = pseudof*Pcont*N/1e3              # pseudofecies P content [kg/d]
  ...
  Wd=Wb+R               # Total mussel dry weight [g]
  Cmyt=(Wd*CcontMyt*N)/1e3    # C content of mussel [kg]
  Nmyt=(Wd*NcontMyt*N)/1e3    # N content of mussel [kg]
