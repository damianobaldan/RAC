...
#' @param G food entering the cage at time series
...
Bass_pop_RKsolver <- function(Param, Temperature, G, Food, IC, times, N){
  ...
  Gapp=G[t]
  Napp=N[t]
  output<-Bass_pop_equations(Param, Napp, Tapp, Gapp, Food, weight[t])
  dw=unlist(output[1])
  k1=timestep*dw
  ...
  output<-Bass_pop_equations(Param, Napp, Tapp$y, Gapp$y, Food, weight[t]+k1/2)
  dw=unlist(output[1])
  k2=timestep*dw;
  ...
  Tapp=approx(seq(from=1,to=tf,by=timestep),Temperature,xout=(t+timestep/2))
  Gapp=approx(seq(from=1,to=tf,by=timestep),G,xout=(t+timestep/2))
  output<-Bass_pop_equations(Param, Napp, Tapp$y, Gapp$y, Food, weight[t]+k2/2)
  dw=unlist(output[1])
  k3=timestep*dw;
  ...
  Gapp=G[t+timestep]
  output<-Bass_pop_equations(Param, Napp, Tapp, Gapp, Food, weight[t]+k3)
  dw=unlist(output[1])
  k4=timestep*dw;
  ...
  output<-Bass_pop_equations(Param, N[t+timestep], Temperature[t+timestep], G[t+timestep], Food, weight[t+timestep])
