...
#' @param IC vector containing initial conditions on weight
#' @param times vector containing integration extremes and timestep
#'
...
Bream_ind_RKsolver <- function(Param, Temperature, G, Food, IC, times){
  ...
  Tapp=Temperature[t]
  Gapp=G[t]
  output<-Bream_ind_equations(Param, Tapp, Gapp, Food, weight[t])
  dw=unlist(output[1])
  k1=timestep*dw
  ...
  output<-Bream_ind_equations(Param, Tapp$y, Gapp$y, Food, weight[t]+k1/2)
  dw=unlist(output[1])
  k2=timestep*dw;
  ...
  Tapp=approx(seq(from=1,to=tf,by=timestep),Temperature,xout=(t+timestep/2))
  Gapp=approx(seq(from=1,to=tf,by=timestep),G,xout=(t+timestep/2))
  output<-Bream_ind_equations(Param, Tapp$y, Gapp$y, Food, weight[t]+k2/2)
  dw=unlist(output[1])
  k3=timestep*dw;
  ...
  Gapp=G[t+timestep]
  output<-Bream_ind_equations(Param, Tapp, Gapp, Food, weight[t]+k3)
  dw=unlist(output[1])
  k4=timestep*dw;
  ...
  output<-Bream_ind_equations(Param, Temperature[t+timestep], G[t+timestep], Food, weight[t+timestep])
