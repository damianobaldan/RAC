...
ClamF_pop_main<-function(userpath,forcings){

rm(list=ls())       # Clean workspace

cat('Clam population bioenergetic model - alternative version\n')
cat(" \n")

# Run the preprocessor for the first time to print to screen parameters and forcing selected
out_pre<-ClamF_pop_pre(userpath,forcings)
...
while (identical(selector,"y")=="TRUE") {
  ...
  cat(paste0(userpath,"/ClamF_population/Inputs\n"))
  cat(" \n")
